...
func (d Datastore) LoadWorkflowRunner(uuid string) (wfr workflow.WorkflowRunner, err error) {
    var config string
    ...
    return workflow.ParseWorkflowRunner(config)
}

// DumpWorkflowRunner ...
// Dump a running `WorkflowRunner`'s state to storage
func (d Datastore) DumpWorkflowRunner(wfr workflow.WorkflowRunner) error {
